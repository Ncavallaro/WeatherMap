...
const PanelClima = () => {
  let urlWeather =
    'https://api.openweathermap.org/data/2.5/weather?appid=69b34db4e81bcec6834b291c4a34f23b&lang=es'
  let pathParamCity = '&q='
  let measurement = '&units=metric'
  let urlForecast =
    'https://api.openweathermap.org/data/2.5/forecast?appid=69b34db4e81bcec6834b291c4a34f23b&lang=es'
  ...
  const getLocation = async (city) => {
    ...
    await fetch(urlForecast)
      .then((response) => {
        if (!response.ok)
        {
          throw new Error('Error al obtener los datos'+ location);
        } 
        return response.json()
      })
      .then((forecastData) => {
  ...
  return (
    <>
      <SelectClima newLocation={getLocation} />
      <InformationWeather
        showData={show}
        loadingData={loading}
        weather={weather}
